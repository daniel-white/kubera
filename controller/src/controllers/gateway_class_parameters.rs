...
pub struct GatewayClassParametersController {
    ...
    state_rx: Receiver<Option<GatewayClassParametersState>>,
...
impl GatewayClassParametersController {
    pub fn new(
        ...
        let (state_tx, state_rx) = channel(None);
        GatewayClassParametersController {
            client,
            state_tx,
            state_rx,
            gateway_class_state_rx,
        }
    }

    pub fn state_rs(&self) -> Receiver<Option<GatewayClassParametersState>> {
        self.state_rx.clone()
    }
    ...
    pub fn run(&self) -> JoinHandle<()> {
        ...
        let mut gateway_class_state_rx = self.gateway_class_state_rx.clone();
                    ...
                    Some(gateway_class_state) => match gateway_class_state.parameter_ref() {
                        ...
                        Some(parameters_ref) => {
                            ...
                            let watcher_config = Config::default().fields(
                                format!("metadata.name={}", parameters_ref.name()).as_str(),
                            );
                            ...
                            select! {
                                _ = Controller::new(parameters_api, watcher_config)
                                    ...
                                    .for_each(|_| ready(()))
                                     => {
                                        // Handle the controller's shutdown signal
                                        break;
                                    },
                                _ = gateway_class_state_rx.changed() => {
                                    continue;
