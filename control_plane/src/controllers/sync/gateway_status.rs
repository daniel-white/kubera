...
pub fn sync_gateway_status(
    ...
    gateway_rx: &Receiver<(ObjectRef, Arc<Gateway>)>,
) {
    ...
    task_builder
        ...
        .spawn(async move {
            loop {
                await_ready!(kube_client_rx, instance_role_rx, gateway_rx)
                    .and_then(async |kube_client, instance_role, (gateway_ref, gateway)| {
                        ...
                        match current_gateway {
                            Some(mut current_gateway) if instance_role.is_primary() => {
                            ...
                            Some(_) => {
                                debug!("Instance is not primary, skipping Gateway status update");
                            }
                            None => {
                                warn!("Failed to retrieve current Gateway status");
                            }
                        }
                    })
                    .run()
